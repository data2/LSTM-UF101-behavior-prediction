...
class VideoLSTMTransformerModel(nn.Module):
    def __init__(self, input_size, hidden_size, num_classes):
        ...
        # 定义LSTM层
        self.lstm = nn.LSTM(2048, hidden_size, batch_first=True)  # ResNet50输出2048维的特征
...
class VideoBehaviorModel:
    def __init__(self, video_folder, input_size=224, hidden_size=256, num_classes=3, batch_size=4,
                 epochs=10, fixed_frame_count=100, device=None):
    ...
    def prepare_data(self):
        """
        加载 'HighJump', 'HighDive', 'Basketball' 三个类别的视频数据
        """
        video_paths, labels = [], []
        categories = ['HighJump', 'Diving', 'Basketball']

        for idx, category in enumerate(categories):
            category_path = os.path.join(self.video_folder, category)
            if not os.path.exists(category_path):
                raise ValueError(f"No '{category}' class found in the dataset")

            for video_name in os.listdir(category_path):
                if video_name.endswith('.avi'):  # 只处理.avi格式的视频
                    video_path = os.path.join(category_path, video_name)
                    video_paths.append(video_path)
                    labels.append(idx)  # 使用索引作为类别标签
    ...
    def train_model(self, model, train_loader, criterion, optimizer):
        model.to(self.device)  # 将模型传输到GPU或CPU
    ...
    def main(self):
        ...
        # 初始化模型
        model = VideoLSTMTransformerModel(input_size=224, hidden_size=256, num_classes=3)  # 多分类3类
        ...
        # 保存模型
        self.save_model(model, 'action_video_behavior_model.pth')
...
if __name__ == '__main__':
    video_folder = '/home/jsdp/UCF-101'  # 你的数据集路径
    model = VideoBehaviorModel(video_folder, device='cuda')  # 使用GPU训练
    model.main()

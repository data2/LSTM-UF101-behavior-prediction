...
class VideoLSTMTransformerModel(nn.Module):
    def __init__(self, input_size, hidden_size, num_classes):
        ...
        input_size: 每帧的输入特征维度（CNN提取特征后的维度）
        ...
        self.resnet = models.resnet18(pretrained=True)  # 也可以使用其他如ResNet50
        self.resnet.fc = nn.Identity()  # 去掉最后的全连接层，保留特征提取部分

        # 定义LSTM层，输入维度为ResNet输出的特征维度
    ...
    def forward(self, x):
        ...
        for i in range(seq_len):
            frame = x[:, i, :, :, :]  # 获取每一帧的图像，形状为 [batch_size, c, h, w]
            frame_features = self.resnet(frame)  # 使用ResNet提取每一帧的特征
            features.append(frame_features.unsqueeze(1))  # 增加维度，形状为 [batch_size, 1, feature_size]

        # 将所有帧的特征堆叠在一起，得到 [batch_size, seq_len, feature_size]
...
class VideoBehaviorModel:
    def __init__(self, video_folder, input_size=224, hidden_size=256, num_classes=101, batch_size=4,
    ...
    def train_model(self, model, train_loader, criterion, optimizer):
        ...
        for epoch in range(self.epochs):
            ...
            for inputs, labels in train_loader:
                inputs, labels = inputs.to(self.device), labels.to(self.device)
                labels = labels.long()  # 确保标签是 LongTensor 类型
                optimizer.zero_grad()
